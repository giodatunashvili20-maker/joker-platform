...
async function initDb() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS room_messages (
      ...
      CHECK (
        ...
        OR (from_user_id IS NULL AND from_bot_id IS NOT NULL)
      )
    );

    /* MATCHMAKING QUEUE (single table, auto-tier) */
    CREATE TABLE IF NOT EXISTS matchmaking_queue (
      ...
      delete_scope TEXT NOT NULL DEFAULT 'last', -- last/all (moshla always ON)
  ...
  // Anti-cheat unique index for replay protection
  await pool.query(`
  ...
  // Migration safety (if old schema existed): add columns if missing
  await pool.query(`
    ALTER TABLE users ADD COLUMN IF NOT EXISTS leave_strikes INT NOT NULL DEFAULT 0;
